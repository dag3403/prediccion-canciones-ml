...
cat_clf = CatBoostClassifier(loss_function='MultiClass', auto_class_weights='Balanced', random_seed=42, verbose=0, thread_count=-1)
pipeline = Pipeline(steps=[('preprocessor', preprocessor), ('classifier', cat_clf)])

cv_strategy = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
param_grid = {'classifier__iterations': [100, 200], 'classifier__depth': [4, 6], 'classifier__learning_rate': [0.05, 0.1]}
...
with open("models/resultados_catboost_2.json", "w", encoding="utf-8") as f:
    json.dump(resultados_dict, f, indent=4, ensure_ascii=False)
print("Pipeline CatBoost Penalizado completado.")