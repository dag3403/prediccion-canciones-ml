...
from sklearn.pipeline import Pipeline  # Asegúrate de tener esta importación arriba
...
if __name__ == "__main__":
    ...
    # 4. Separar Features (X) incluyendo texto y Target (y)
    X = df_clean.drop(columns=['artist_name'], errors='ignore')
    y = df_clean['artist_name']

    # 5. Dividir datos: 90% entrenamiento, 10% test (estratificado)
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.10, random_state=42, stratify=y
    )

    # 6. Definición de estrategia híbrida basada en la mediana
    conteo_clases = y_train.value_counts()
    mediana_clases = int(conteo_clases.median())
    
    # Creamos un diccionario de estrategia de muestreo dinámico hacia la mediana para SMOTETomek
    sampling_strategy_dict = {
        clase: max(conteo, mediana_clases) 
        for clase, conteo in conteo_clases.items() 
        if conteo < mediana_clases
    }
    
    print(f"\nMediana de muestras por clase seleccionada para balanceo híbrido: {mediana_clases}")

    # 7. Construcción del Pipeline de Modelado Híbrido con Texto
    numeric_features = X_train.select_dtypes(include=[np.number]).columns.tolist()

    # Pipelines específicos para procesar texto de forma estructurada
    ...
    # Balanceo Híbrido (SMOTE para sobremuestreo controlado + Tomek Links para limpieza de ruido)
    ...
    # Usamos ImbPipeline para asegurar que el remuestreo ocurra de forma segura dentro de la validación cruzada
    ...
    # 8. Optimización de hiperparámetros con 10-Fold CV (Optimizando f1_macro)
    ...
    print("\nIniciando optimización de hiperparámetros con GridSearchCV (10-Fold) y Balanceo Híbrido...")
    grid_search = GridSearchCV(
        estimator=pipeline,
        param_grid=param_grid,
        cv=cv_strategy,
        scoring='f1_macro',  # Optimiza para dar balance a todas las clases
        n_jobs=-1
    )
    ...
    # 9. Evaluación y métricas en el conjunto de test
    ...
    # Nota: El OOB score directo del Random Forest no está disponible al usar pipelines con pasos de remuestreo previos
    oob_score_val = None
    oob_error_val = None

    conf_matrix = confusion_matrix(y_test, y_pred)

    # --- 10. GUARDAR RESULTADOS EN JSON DENTRO DE LA CARPETA models ---
    ...
    json_path = "models/resultados_modelo_random_forest_3.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(resultados_dict, f, indent=4, ensure_ascii=False)
    
    print(f"\nResultados guardados exitosamente en: {json_path}")

    # 11. Impresión de métricas en consola
    print("\n================ MÉTRICAS FINALES EN TEST (MIXTO + HÍBRIDO) ================")
