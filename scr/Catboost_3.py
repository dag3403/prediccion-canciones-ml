...
sampling_strategy_dict = {clase: max(conteo, mediana_clases) for clase, conteo in conteo_clases.items() if conteo < mediana_clases}
...
cat_clf = CatBoostClassifier(loss_function='MultiClass', random_seed=42, verbose=0, thread_count=-1)
...
param_grid = {'classifier__iterations': [100, 200], 'classifier__learning_rate': [0.05, 0.1]}
...
with open("models/resultados_catboost_3.json", "w", encoding="utf-8") as f:
    json.dump(resultados_dict, f, indent=4, ensure_ascii=False)
print("Pipeline CatBoost SMOTETomek Mediana completado.")