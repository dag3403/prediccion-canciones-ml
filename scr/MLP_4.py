...
weights_q75_dict = {cls: float(q75_count / count) if count > 0 else 1.0 for cls, count in class_counts.items()}
sample_weights_train = np.array([weights_q75_dict[val] for val in y_train])
...
resultados_dict = {
    "estrategia": "MLP Caso 4: Balanceo al Cuantil 75",
...
with open("models/resultados_mlp_4.json", "w", encoding="utf-8") as f:
    json.dump(resultados_dict, f, indent=4, ensure_ascii=False)
print("MLP Caso 4 (Balanceo al Cuantil 75) completado y guardado.")