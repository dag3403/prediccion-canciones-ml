...
weights_median = {cls: float(median_count / count) if count > 0 else 1.0 for cls, count in class_counts.items()}
...
svc_clf = SVC(probability=True, class_weight=weights_median, random_state=42)
pipeline = Pipeline(steps=[('preprocessor', preprocessor), ('classifier', svc_clf)])
...
resultados_dict = {
    "estrategia": "SVM Caso 3: Balanceo a la mediana",
...
with open("models/resultados_svm_3.json", "w", encoding="utf-8") as f:
    json.dump(resultados_dict, f, indent=4, ensure_ascii=False)
print("SVM Caso 3 (Balanceo a la mediana) completado y guardado.")