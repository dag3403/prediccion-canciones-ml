...
quantile_75_count = class_counts.quantile(0.75)
weights_q75 = {cls: (quantile_75_count / count) if count > 0 else 1.0 for cls, count in class_counts.items()}
sample_weights_caso4 = np.array([weights_q75[cls] for cls in y_train])
...
grid_search.fit(X_train, y_train, classifier__sample_weight=sample_weights_caso4)
...
with open("models/resultados_naive_bayes_4.json", "w", encoding="utf-8") as f:
