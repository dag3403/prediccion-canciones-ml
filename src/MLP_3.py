...
weights_median_dict = {cls: float(median_count / count) if count > 0 else 1.0 for cls, count in class_counts.items()}
sample_weights_train = np.array([weights_median_dict[val] for val in y_train])
...
resultados_dict = {
    "estrategia": "MLP Caso 3: Balanceo a la mediana",
...
with open("models/resultados_mlp_3.json", "w", encoding="utf-8") as f:
    json.dump(resultados_dict, f, indent=4, ensure_ascii=False)
print("MLP Caso 3 (Balanceo a la mediana) completado y guardado.")