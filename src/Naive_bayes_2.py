...
weights_balanced = compute_class_weight('balanced', classes=np.unique(y_train), y=y_train)
sample_weights_caso2 = np.array([weights_balanced[cls] for cls in y_train])
...
grid_search.fit(X_train, y_train, classifier__sample_weight=sample_weights_caso2)
...
resultados_dict = {
    "estrategia": "Caso 2: Penalización de errores",
...
with open("models/resultados_naive_bayes_2.json", "w", encoding="utf-8") as f:
    json.dump(resultados_dict, f, indent=4, ensure_ascii=False)
print("Caso 2 (Penalizado) completado y guardado.")